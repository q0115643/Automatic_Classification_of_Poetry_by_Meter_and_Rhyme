...
def evaluate(parameters, f_eval, raw_sentences, parsed_sentences,
    ...
    # Global accuracy
    print "Accuracy Per Syllable %.5f%% (%i/%i)" % (
            100. * count.trace() / max(1, count.sum()), count.trace(), count.sum()
    )

    print "Accuracy Per Line %.5f%% (%i/%i)" % (
        100. * correct_sentence_cnt / max(1, all_sentence_cnt), correct_sentence_cnt, all_sentence_cnt
    )
